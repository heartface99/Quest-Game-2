...
public class Monsterlist{
    /*
        Class that holds information about all the possible monsters in the game
    */
    ...
    public void init_Monster(){
        // every game we create new monsters from a predetermined list of monsters
        monsters.clear();
    ...
    public ArrayList<Monster> matchLevel(int level, int num_monsters){
        // get monsters that match the highest level of the heroes
        ...
        for(i = 0; i < monsters.size(); i++){
            ...
            if(current.getLevel()==level){
                monsterlist.add(new Monster(current.getname(), current.getLevel(), current.getDamage(), current.getDef(), current.getDodge(), current.getType()));
            }
        }
